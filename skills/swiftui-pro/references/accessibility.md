...
- If the user has “Reduce Motion” enabled, replace large, motion-based animations with opacity instead. Read the `accessibilityReduceMotion` environment value and gate animations with the `.animation(reduceMotion ? nil : .default, value:)` pattern.
