...
**Input:** An epic number (e.g., `/epic-detail 1` for Epic 1).
...
2. Read `docs/PROJECT-BRIEF.md` — if it doesn't exist, tell the user to run `/project-brief` first and stop.
...
10. Tell the user: _"Epic {N} document created at docs/epics/EPIC-{number}.md with {X} open questions. Answer the questions in the document, then run `/epic-detail {N}` again."_
...
9. Report status:
   - If still under 90%: _"Updated. Confidence now at {X}%. {Y} questions remaining. Answer them and run `/epic-detail {N}` again."_
