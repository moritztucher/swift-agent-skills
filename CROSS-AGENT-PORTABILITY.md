...
- **Skill-routing rule in `AGENTS.md`:** agent-neutral instruction to check the catalog and load the matching framework skill before implementing. Backs up auto-triggering on every client — Claude Code (where the curated `skillOverrides` tiering in `settings.json.example` keeps 72 framework-literal skills name-only), Codex (8k-char description budget), Gemini, and Kiro.
